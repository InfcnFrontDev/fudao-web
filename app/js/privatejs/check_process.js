 ...
 * @param {Object} info
 */
function fun_pathogeny(info, list) {
    symptom = list;
...
function fun_pie(data, position, type) {
    ...
    pieChart.on('pieSelected', function (params) {
        ...
        if (params.selected["0"][2] && params.target == "更多") {
            ...
            var div_list = document.getElementById("div_list");
            div_list.innerHTML = "";
            for (var times = 0; times < symptom.length; times++) {
                if (symptom[times].organ == params.target) {
                    console.log(times);
                    break;
                }
            }
            var _zhengzhuang = symptom[times].symptom;
...
function fun_select(obj, type) {
    if (type == 'right') {
        ...
        if (!boo) {
            if (div_list_selected.children.length >= 12) {
                return;
            }
            choosed.push(obj.innerText);
            var _div = document.createElement("div");
            _div.style.float = "left";
            _div.innerHTML = '<a text ="' + obj.innerText + '" style="font-size: 14px;color:#000000;background:#D9ECF3;margin:5px;padding: 4px;display: inline-block" onclick ="fun_select(this)">' + obj.innerText + '&nbsp;&nbsp;<span style="color:#D6D5D1;background: #fff;border-radius: 30px;">&nbsp;x&nbsp;</span></a>';
...
//     $.ajax({
//         type: "get",
//         url: "http://192.168.10.69:9191/api/DiagnosisApi/getRecentThingList",
//         headers: {
//             authorization: token
//         },
//         success: function (res) {
    ...
    for(var index in res.obj){
        ...
        _p.innerHTML = '<p style="text-align: left;margin: 0 40px;border-bottom: 1px solid #D8D8D8;padding-bottom: 10px;font-size: 15px"><span style="display: inline-block;width:90%;">'+ res.obj[index]+'</span><input type="checkbox"  name="category" value="今日话题"  onclick="my_func()"/></p>';
